...
<html>
	...
	<body>
	 ...
	 %> 
		...
		<form id="categoryForm" action="ProductServlet?action=getPageByQuery&target=productMainUpload" method="post">
			...
			<table id="productTable" class="table table-striped table-bordered table-hover">
				...
				<%
					int kid = product.getKID();
							...
							Kind category = service.getCategoryById(kid);
				%>
...
<form id="productForm2" action="ProductServlet?action=getPageByQuery&target=productMainUpload" method="post">

			<div id="pageInfoDiv">
			
	<a id="first" class="btn btn-default btn-sm" href='ProductServlet?action=getPageByQuery&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=1'>首页</a>
    
    
    <a id="previous" class="btn btn-default btn-sm" href='ProductServlet?action=getPageByQuery&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getPreviousPage()%>'> 上一页</a>
      <a id="next" class="btn btn-default btn-sm" href='ProductServlet?action=getPageByQuery&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getNextPage()%>'> 下一页</a>
    
   
     <a id="last" class="btn btn-default btn-sm" href='ProductServlet?action=getPageByQuery&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getTotalPageCount()%>'>尾页</a>
   ...
   <span>共<%=pageInfo.getTotalRecordCount()%>条记录 </span>
    ...
    <span>页 </span>
		...
		<script src="resources/js/jQuery-confirm/jquery-confirm.js" type="text/javascript" charset="utf-8"></script>
	...
	return afterTrimTxt;
			...
			function deleteProductById(pid){
				$.confirm({
				    ...
				    confirm: function(){
				       	//发请求
				       	location.assign("ProductServlet?action=delete&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&pid=" + pid);
				    },
				    cancel: function(){
			...
			function getOneForUpload(pid){
				...
				location.assign("ProductServlet?action=getOneForUpload&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&pid=" + pid);
			}
