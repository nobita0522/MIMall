		...
		<div class="text-center categoryTableDiv">
			...
			<table id="productTable" class="table table-striped table-bordered table-hover">
				...
				%>
						...
						<td>
							<button onclick="deleteProductById(<%=product.getPID() %>)" class="btn btn-danger btn-xs">删除</button>
						</td>
					</tr>
				<%
					}
...
<form id="productForm2" action="ProductServlet?action=getPageByQuery&target=productMain" method="post">
	...
	<a id="first" class="btn btn-default btn-sm" href='ProductServlet?action=getPageByQuery&target=productMain&searchCondition=${requestScope.searchCondition}&requestPage=1'>首页</a>
    
    
    <a id="previous" class="btn btn-default btn-sm" href='ProductServlet?action=getPageByQuery&target=productMain&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getPreviousPage()%>'> 上一页</a>
      <a id="next" class="btn btn-default btn-sm" href='ProductServlet?action=getPageByQuery&target=productMain&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getNextPage()%>'> 下一页</a>
	...
	return afterTrimTxt;
			...
			function deleteProductById(pid){
				$.confirm({
				    ...
				    confirm: function(){
				       	//发请求
				       	location.assign("ProductServlet?action=delete&target=productMain&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&pid=" + pid);
				    },
				    cancel: function(){
