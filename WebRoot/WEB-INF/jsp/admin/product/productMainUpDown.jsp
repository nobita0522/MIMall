<%@ page language="java"
	import="java.util.*,com.mimall.vo.*,com.page.PageInfo,com.mimall.service.impl.KindServiceImpl,com.mimall.util.ProductDictionary"
	pageEncoding="UTF-8"%>
<%
...
<link rel="stylesheet" type="text/css"
	href="resources/css/jQuery-confirm/jquery-confirm.css" />
<style type="text/css">
body {
			...
			<table id="productTable" class="table table-striped table-bordered table-hover">
				...
				<%
											int kid = product.getKID();
															...
															Kind kind = service.getCategoryById(kid);
										%>
						<td><%=kind.getCname()%></td>
...
<form id="productForm2" action="ProductServlet?action=getPageByQuery&target=productMainUpDown" method="post">

			<div id="pageInfoDiv">
			
	<a id="first" class="btn btn-default btn-sm" href='ProductServlet?action=getPageByQuery&target=productMainUpDown&searchCondition=${requestScope.searchCondition}&requestPage=1'>首页</a>
    
    
    <a id="previous" class="btn btn-default btn-sm" href='ProductServlet?action=getPageByQuery&target=productMainUpDown&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getPreviousPage()%>'> 上一页</a>
      <a id="next" class="btn btn-default btn-sm" href='ProductServlet?action=getPageByQuery&target=productMainUpDown&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getNextPage()%>'> 下一页</a>
		...
		<script type="text/javascript">
			...
			function productDown(pid){
				$.confirm({
				    ...
				    confirm: function(){
				       	//发请求
				       	location.assign("ProductServlet?action=productDown&target=productMainUpDown&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&pid=" + pid);
				    },
				    cancel: function(){
			...
			function productUp(pid){
				$.confirm({
				    ...
				    confirm: function(){
				       	//发请求
				       	location.assign("ProductServlet?action=productUp&target=productMainUpDown&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&pid=" + pid);
				    },
				    cancel: function(){
